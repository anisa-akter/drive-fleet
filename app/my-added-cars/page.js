        ...
        <Container className="space-y-8">
          ...
          {!loading && cars.length === 0 ? (
            <div className="lux-card rounded-2xl p-6 text-sm text-black/70">
              You have not added any cars yet. Add your first car to start
              earning.
            </div>
          ) : null}
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {cars.map((car) => {
              ...
              return (
                <div
                  key={carId}
                  className="lux-card flex h-full flex-col rounded-2xl p-5"
                >
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
              <div className="lux-card w-full max-w-md rounded-2xl p-6">
                ...
                <p className="mt-2 text-sm text-black/70">
                  This will permanently remove {selectedCar.name} from
                  DriveFleet.
                </p>
                {actionMessage ? (
                  <p className="mt-3 text-sm text-error">{actionMessage}</p>
                ) : null}
                <div className="mt-6 flex gap-3">
                  <button
                    type="button"
                    className="btn btn-outline-drive rounded-full"
                  ...
                  <button
                    type="button"
                    className="btn btn-drive rounded-full"
